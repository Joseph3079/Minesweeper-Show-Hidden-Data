...
(function () {
    ...
    var defaultSettings = {dailyLoginConfirmation:false,eliteDowngradeConfirmation:true};
    var currSettings = {};
    var settingAdded = false;
    if(JSON.parse(localStorage.getItem('extraSettings'))==null) {localStorage.setItem('extraSettings',JSON.stringify(defaultSettings)); currSettings = defaultSettings;}
    else {
        currSettings = JSON.parse(localStorage.getItem('extraSettings'));
        for(var i in defaultSettings) {
            if(currSettings[i] == undefined) {
                currSettings[i] = defaultSettings[i];
                settingAdded = true;
            }
        }
    }
    if(settingAdded) {localStorage.setItem('extraSettings', JSON.stringify(currSettings));}

    const mmrToRating = e => !e || e <= 850 ? 0 : e < 1100 ? Math.floor((e - 850) / 5) : Math.floor((e - 1e3) / 2);
    const xmmrs = {};

    const waitForSettings = function() {
        if (document.querySelectorAll('div.form-group').length > 0) {
            var formGroupList = document.querySelectorAll('div.form-group');
            var settingsLoaded = false;
            var settingsIndex = -1;
            for(var i=0; i<formGroupList.length; i++) {
                if(formGroupList[i].innerText.indexOf("Daily login") > -1) {
                    settingsLoaded = true;
                }
                else if(formGroupList[i].innerText.indexOf("Return to the game") > -1) {
                    settingsLoaded = true;
                    settingsIndex = i;
                    break;
                }
            ...
            if(settingsLoaded) {
                if(settingsIndex > -1) {
                    var settingsText = ['Daily login reward confirmation','Elite quest downgrade confirmation'];
                    currNode = document.createElement('div');
                    for(i in currSettings) {
                        currNode.innerHTML += '<div class="form-group"><label class="col-xs-4 control-label"></label><div class="col-xs-8"><label class="'+i+'"><input id="'+i+'" type="checkbox" onclick=toggleSetting("'+i+'")>&nbsp;&nbsp;'+settingsText[currIndex]+'</label></div></div>';
                        currIndex++;
                    }
                    formGroupList[settingsIndex].before(currNode);
                    for(i in currSettings) {
                        document.getElementById(i).checked = currSettings[i];
                    }
                }
            }
            else {
    ...
    window.toggleSetting = function(setting) {
        console.log(setting, currSettings);
        if(document.getElementById(setting).checked) {currSettings[setting]=true;}
        else {currSettings[setting]=false;}
        localStorage.setItem('extraSettings', JSON.stringify(currSettings));
    }
    var lastSettingsLoad = Date.now();
    navigation.addEventListener("navigate", e => {
       if(e.destination.url.indexOf('settings') > -1 && Date.now()-lastSettingsLoad > 100) {
    ...
    const showConfirmDialog = function(buttonToClick) {
        jQuery("#ConfirmDialog").empty().append(jQuery("<div>", {
                    class: "modal-dialog sure-dialog",
                    role: "document"
                }).append(jQuery("<div>", {
                        class: "modal-content"
                    }).append(jQuery("<div>", {
                            class: "modal-body sure-modal-body"
                        }).append(jQuery("<div>", {
                                class: "text-center"
                            }).append("Are you sure?")), jQuery("<div>", {
                            class: "modal-footer"
                        }).append(jQuery("<div>", {
                                class: "text-center"
                            }).append(jQuery("<button>", {
                                    id: "#ConfirmDialog_cancel_btn",
                                    type: "button",
                                    class: "btn btn-default",
                                    "data-dismiss": "modal"
                                }).html("Cancel"), "", "&nbsp;&nbsp;", jQuery("<button>", {
                                    id: "#ConfirmDialog_ok_btn",
                                    type: "button",
                                    class: "btn btn-primary sure-ok-btn"
                                }).on("click", () => {jQuery("#ConfirmDialog").modal("hide");
                                                      document.getElementById(buttonToClick).style.visibility='';
                                                      document.getElementById(buttonToClick).click();
                                                      document.getElementById(buttonToClick).style.visibility='hidden';
                                                      if(buttonToClick.substr(0,9) == 'downgrade') {setTimeout(waitForQuests,500);}
                                                     }).html("OK"))))));
        jQuery("#ConfirmDialog").modal("show");
    }
    const waitForQuests = function() {
        var currList = document.querySelectorAll('tr');
        var profileLoaded = false;
        for(var i=0; i<currList.length; i++) {
            if(currList[i].id.substring(0,9) == 'quest_row') {
                profileLoaded = true;
                break;
            }
        }
        if(profileLoaded) {
            ...
            if(!alreadyAdded && currSettings.eliteDowngradeConfirmation) {
                ...
                for(i=0; i<currList.length; i++) {
                    if(currList[i].id.substr(0,9) == 'downgrade') {
                        currList[i].style.visibility = 'hidden';
                        currList[i].style.height = '0px';
                        currList[i].style.width = '0px';
                        currNode = document.createElement('span');
            ...
            setTimeout(waitForQuests,10);
        }
    }
    var lastProfileLoad = Date.now();
    navigation.addEventListener("navigate", e => {
       if(e.destination.url.indexOf('/quests') > -1 && e.destination.url[e.destination.url.length-1] == 's' && Date.now()-lastProfileLoad > 100) {
           lastProfileLoad = Date.now();
           setTimeout(waitForQuests,100);
        }
    });
    if(window.location.href.indexOf('/quests') > -1 && window.location.href[window.location.href.length-1] == 's') {
        waitForQuests();
    }
    ...
    const showStatistics = function(s) {
        jQuery('#profileStatistics').empty();
        ...
        jQuery('#profileStatistics').append(
            jQuery('<div>', {
                class : 'row'
            }).append(
                jQuery('<div>', {
    ...
    const waitForProfileHTML = function() {
        if(document.querySelectorAll('profileStatistics').length == 0 && document.querySelectorAll('div.col-xs-4').length > 0) {
            ...
            for(var i=0; i<currList4.length; i++) {
                if(currList4[i].innerText == "Information:") {
                    profileLoaded = true;
                    break;
                }
            }
